...
var uiduck = function (o, e) {
    var uiduck = {
        ...
        setHead: function (uiduck) {
            ...
            for (var i = 0; i < uiduck.fieldOptions.length; i++) {
                ...
                if (uiduck.fieldOptions[i].selection) {
                    g += "<th class='" + uiduck.style.thClass + "'  style='font-weight:blod;color: #fff;'><label class='el-checkbox'><span class='el-checkbox__input'><span class='el-checkbox__inner'></span><input type='checkbox' aria-hidden='false' class='el-checkbox__original' value=''></span><!----></label></th>"
                    continue;
                }
                if (i < mergeIndex) {
                    continue;
                ...
                if (uiduck.fieldOptions[i].width != undefined) {
                    if (uiduck.fieldOptions[i].width.indexOf("%") != -1) {
                        g += "<th class='" + uiduck.style.thClass + "' colspan='" + colspan + "' style='width:" + uiduck.fieldOptions[i].width + ";font-weight:blod;color: #fff;'>" + uiduck.fieldOptions[i].title + "</th>"
                    } else if (uiduck.fieldOptions[i].width.indexOf("px") != -1) {
                        if(uiduck.fieldOptions[i].hide){
                           g += "<th hidden class='" + uiduck.style.thClass + "' colspan='" + colspan + "' style='width:" + uiduck.fieldOptions[i].width + ";font-weight:blod;color: #fff;'>" + uiduck.fieldOptions[i].title + "</th>"
                        }else {
                            g += "<th class='" + uiduck.style.thClass + "' colspan='" + colspan + "' style='width:" + uiduck.fieldOptions[i].width + ";font-weight:blod;color: #fff;'>" + uiduck.fieldOptions[i].title + "</th>"
                        }
                    } else if (uiduck.fieldOptions[i].width.indexOf("px") == -1 && uiduck.fieldOptions[i].width.indexOf("%") == -1) {
                        g += "<th class='" + uiduck.style.thClass + "' colspan='" + colspan + "'  style='width:" + uiduck.fieldOptions[i].width + "px;font-weight:blod;color: #fff;'>" + uiduck.fieldOptions[i].title + "</th>"
                    }
                } else {
                    g += "<th class='" + uiduck.style.thClass + "'  colspan='" + colspan + "'  style='font-weight:blod;color: #fff;'>" + uiduck.fieldOptions[i].title + "</th>"
                }
            }
            if (uiduck.rightTool) {
                        ...
                        for (var keys in uiduckJL[i]) {
                            if (keys === uiduckFO[k].key) {
                                if (uiduckFO[k].type == 'map') {
                                    ...
                                    if(uiduckFO[k].hide){
                                        h += "<td hidden  class=" + e.style.tdClass + ">" + uiduckJL[i][keys] + "</td>";
                                    }else {
        ...
        hideLoading: function (e) {
            setTimeout(function () {
                $("#" + uiduck.templateId + "-ud-loading").css('display', 'none');
            ...
            if (options.limit != undefined) {
                ...
                if (e.pageOptions.index > 0) {
                    if (e.pageOptions.layout.indexOf('home') != -1) {
                        g += '<a onclick="' + o + '.firstPage();" >' + e.language.options.udHome + '</a>';
                    }
                    if (e.pageOptions.layout.indexOf('prev') != -1) {
                        g += '<a onclick="' + o + '.prevPage();" >' + e.language.options.udPrev + '</a>';
                    }
                } else {
                    if (e.pageOptions.layout.indexOf('home') != -1) {
                ...
                if(e.pageOptions.em ==undefined){
                    for (var i = 0; i < count; i++) {
                        if (count > 8) {
                            if (i == 1) {
                                if (parseInt(e.pageOptions.index) > 4) {
                                    g += '<a  onclick="' + o + '.expandPage(' + parseInt(parseInt(e.pageOptions.index) - 4) + ',0)">...</a>';
                                } else {
                                    if (parseInt(e.pageOptions.index) == i) {
                                        ...
                                        g += '<a  onclick="' + o + '.skipPage(' + i + ')">' + parseInt(i + 1) + '</a>';
                                    }
                                }
                            } else if (i == 7) {
                                if (i > parseInt(parseInt(e.pageOptions.index) + 2)) {
                                    g += '<a  onclick="' + o + '.expandPage(' + i + ',1)">...</a>';
                                } else {
                                ...
                                if (increment < 4) {
                                    if (parseInt(e.pageOptions.index) + increment - 6 < parseInt(e.pageOptions.index) - increment < i) {
                                        if (parseInt(e.pageOptions.index) + increment - 5 > i) {} else {
                                            if (i > parseInt(parseInt(e.pageOptions.index) + 2)) {
                ...
                if (count > 1) {
                    if (e.pageOptions.index <= parseInt(count - 2)) {
                        ...
                        if (e.pageOptions.layout.indexOf('last') != -1) {
                            g += '<a onclick="' + o + '.lastPage();" >' + e.language.options.udLast + '</a>';
                        }
                    } else {
                        if (e.pageOptions.layout.indexOf('next') != -1) {
                ...
                if (e.pageOptions.layout.indexOf('jump') != -1) {
                    g += '<span class="uiduck-page-skip">' + e.language.options.udGo + '<input id="ud-page-skip-text" type="text" min="1" value="1" class="uiduck-page-input">' + e.language.options.udPage + '<button type="button" class="uiduck-page-btn" onclick="' + o + '.skipPage()">' + e.language.options.udSub + '</button></span>'
                }
            } else {
    ...
    return {
        base:uiduck,
        setData: uiduck.setData,
        skipPage: uiduck.skipPage,
        nextPage: uiduck.nextPage,
        lastPage: uiduck.lastPage,
        ...
        getOptions:uiduck.getOptions,
        setHead:uiduck.setHead,
        setOptions:uiduck.setOptions
    }
}