...
;!(function ( window, factory ) {
    ...
    var jeDialog = function(options) {
        ...
        var is= that.config.cell == "" || that.config.cell == undefined;
        that.jeidx = is ? ++jeBox.jeidx : that.config.cell.replace(/[#.]/, "");
        that.initView(is);
    };
    var jefn = jeDialog.prototype;

    //初始化并加载弹层骨架
    jefn.initView = function (is) {
        ...
        //判断ID是否已经存在
        if (opts.cell && $("#" + doms[0] + that.jeidx)[0]) jeBox.close(that.jeidx);
        switch (isType) {
            case lays[0]:
                opts.type = lays[0];
    ...
    jefn.creatBox = function(callback) {
        ...
        //创建默认的弹出层内容模板title="&#20851;&#38381;" title="最大化"
        var templates = '<span class="jeBox-headbtn"><a href="javascript:;" class="jeBox-maxbtn" ></a><a href="javascript:;" class="jeBox-close" ></a></span>' + '<div class="jeBox-header"></div>' + '<div class="jeBox-content" style="padding:' + (paddings != "" ? paddings : 0) + ';">'+opts.content+'</div>' + btnHtml;
        //创建弹窗外部DIV
